...
Each `EnvironmentAsset` is project-owned and records an HTTP(S) address plus a lifecycle type: `development`, `testing`, or `production`. It owns one or more ordered `EnvironmentAccount` records containing an account name and password. In the current controlled preview these passwords are limited by product policy to non-sensitive, self-service test accounts, stored as plain text in SQLite, and returned unchanged by the environment API so the UI can display them without masking. Existing account-only rows migrate with an empty password. Tokens, private keys, and production deployment credentials remain outside this model and stay in GitLab CI or the target infrastructure.

Each `KnowledgeAsset` is a project-owned Markdown document. Its title and authored Markdown content are stored unchanged in SQLite. The client uses Milkdown for live editing and maps inline tokens using `[[asset type：record id]]` to atomic controls that show current asset metadata and serialize back to the same Markdown token. Supported targets are repository assets, project members, environment assets, and other knowledge assets. Resolution is restricted to the current project. The Markdown remains the source of truth: deleting or moving a target does not rewrite authored content, and the API returns the token as unresolved instead of silently pointing elsewhere.
