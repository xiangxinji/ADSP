...
const props = defineProps<{
  ...
  activeTab: 'requirements' | 'assets' | 'workflows'
...
const workflowsPath = computed(() => `${projectPath.value}/workflows`)
...
<template>
  <nav class="breadcrumbs" aria-label="当前位置">
    ...
    <span v-else-if="activeTab === 'workflows'" aria-current="page">工作流</span>
    <template v-else>
  ...
  <section class="workspace-heading">
    ...
    <div class="workspace-stats">
      ...
      <span><strong>{{ workspace.workflows.length }}</strong>工作流</span>
  ...
  <nav class="tabs" aria-label="项目模块">
    ...
    <NuxtLink :to="workflowsPath" :class="{ active: activeTab === 'workflows' }" :aria-current="activeTab === 'workflows' ? 'page' : undefined">工作流</NuxtLink>
