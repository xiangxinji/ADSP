  ...
  <div class="app-frame">
    ...
    <AppDialog :open="showCreateDialog" title="新增用户" overline="NEW USER" :busy="saving" @request-close="requestCloseCreateDialog">
        <form id="create-user-form" @submit.prevent="createUser">
          ...
          <AppFormField field-id="user-password" label="密码" hint="密码需为 8–128 个字符，系统仅保存不可逆哈希。"><div class="secret-input"><AppInput id="user-password" v-model="form.password" required minlength="8" maxlength="128" :type="showCreatePassword ? 'text' : 'password'" autocomplete="new-password" placeholder="输入初始密码" /><AppButton variant="plain" :icon="showCreatePassword ? 'eye-off' : 'eye'" @click="showCreatePassword = !showCreatePassword">{{ showCreatePassword ? '隐藏' : '显示' }}</AppButton></div></AppFormField>
    ...
    <AppDialog :open="showPasswordDialog" :title="passwordUser?.hasPassword ? '重设密码' : '设置密码'" overline="USER PASSWORD" :busy="saving" @request-close="requestClosePasswordDialog">
      <form id="update-password-form" @submit.prevent="updatePassword">
        ...
        <AppFormField field-id="reset-password" label="新密码" hint="密码需为 8–128 个字符，系统仅保存不可逆哈希。"><div class="secret-input"><AppInput id="reset-password" v-model="passwordForm.password" required autofocus minlength="8" maxlength="128" :type="showResetPassword ? 'text' : 'password'" autocomplete="new-password" placeholder="输入新密码" /><AppButton variant="plain" :icon="showResetPassword ? 'eye-off' : 'eye'" @click="showResetPassword = !showResetPassword">{{ showResetPassword ? '隐藏' : '显示' }}</AppButton></div></AppFormField>
