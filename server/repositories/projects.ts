...
export const listProjectSummaries = (): ProjectSummary[] => (useDatabase().prepare(`
  SELECT p.*,
    ...
    (SELECT COUNT(*) FROM workflow_definitions w WHERE w.project_id = p.id) AS workflow_count,
    ...
    (SELECT COUNT(*) FROM ai_interface_assets ai WHERE ai.project_id = p.id) AS ai_interface_count
...
`).all() as (ProjectRow & {
  ...
  aiInterfaceCount: Number(row.ai_interface_count),
}))
